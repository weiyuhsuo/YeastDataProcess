#!/usr/bin/env python3
"""从SC基因组中提取peak_start到TATA，以及TATA到peak_end的序列
...
FASTA_PATH = Path('ex/SC/genomic_without_mitochonria_chrname.fna')
OUT_FASTA = Path('ex/SC/peak_TATA_regions.fa')
OUT_TSV = Path('ex/SC/peak_TATA_regions.tsv')
    ...
    with OUT_FASTA.open('w') as fout_fa, OUT_TSV.open('w') as fout_tsv:
        ...
        for g in GENES:
            ...
            for region_name, start, end in regions:
                ...
                print(f"{g['common_name']} ({g['std_name']}) - {region_name}: [{start}, {end}) = {len(subseq)} bp")
