...
# ==========================================================================
class _LogWriter:
    def __init__(self, path, also_console=False):
...
def _setup_logging():
    ...
    writer = _LogWriter(log_file, also_console=False)
    sys.stdout = writer
    sys.stderr = writer
    return writer
...
# 数据与输出路径（固定绝对路径，便于复现）
DATA_DIR = "/home/rhys/YeastDataProcess/KM/numpy/data"
OUTPUT_BASE_DIR = "/home/rhys/YeastDataProcess/KM/numpy/output"
RUN_TIMESTAMP = datetime.now().strftime("run_%Y%m%d_%H%M%S")
OUTPUT_DIR = os.path.join(OUTPUT_BASE_DIR, RUN_TIMESTAMP)

# 表达矩阵与条件表
EXPR_FILE = f"{DATA_DIR}/KM数据_matrix.csv"
COND_FILE = f"{DATA_DIR}/KM数据第二次整理版.xlsx"

# 条件编码固定映射
FIXED_MAPPING_FILE = f"{DATA_DIR}/encoding_mapping_info.json"

# matrix 与 summit 配置（四个样本）
MATRIX_SUMMIT_LIST = [
    {"matrix": f"{DATA_DIR}/matrix_C1.csv", "summit": f"{DATA_DIR}/C-1.dedup_peaks.narrowPeak"},
    {"matrix": f"{DATA_DIR}/matrix_C3.csv", "summit": f"{DATA_DIR}/C-3.dedup_peaks.narrowPeak"},
    {"matrix": f"{DATA_DIR}/matrix_O2.csv", "summit": f"{DATA_DIR}/O-2.dedup_peaks.narrowPeak"},
    {"matrix": f"{DATA_DIR}/matrix_O3.csv", "summit": f"{DATA_DIR}/O-3.dedup_peaks.narrowPeak"},
]

# 可选：外部peak标签文件（如无则置为None，会写入-1占位）
LABELS_FILE = None

# 是否使用链特异窗口
STRAND_SPECIFIC = True

# 输出是否按链拆分（expr_pos/expr_neg分开保存），如不需要可设为False
...
GENE_INFO_FILE = None  # 当前流程使用GFF直接构建基因映射

# 物种GFF注释文件（用于获取基因坐标/TSS）
GFF_FILE = '/home/rhys/YeastDataProcess/KM/GCA_001854445.2_ASM185444v2/GCA_001854445.2_ASM185444v2_genomic.gff'  # 保留路径但不再用于位置
FEATURE_TABLE_FILE = f"{DATA_DIR}/GCA_001854445.2_ASM185444v2_feature_table.txt"
...
TSS_BED_FILE = None  # 现由GFF_FILE提供
...
def load_gene_positions(gene_mapping):
    """仅使用 feature_table 提取基因坐标/TSS（0-based），不再回退 GFF。缺失则报错。"""
    if not FEATURE_TABLE_FILE or not os.path.exists(FEATURE_TABLE_FILE):
        raise FileNotFoundError(f"找不到feature_table: {FEATURE_TABLE_FILE}")

    print(f"从feature_table加载TSS位置: {FEATURE_TABLE_FILE}")
    ft = pd.read_csv(
        FEATURE_TABLE_FILE,
        sep='\t',
        dtype=str,
    ...
    # 清理列名（去掉开头的#和空白）
    ft.columns = [c.lstrip('#').strip() for c in ft.columns]
    gene_rows = ft[ft['feature'] == 'gene'].copy()
    gene_pos = []
    for _, row in gene_rows.iterrows():
        try:
            ...
            continue
        # 染色体命名优先使用 genomic_accession（如 CP015054.1），与 narrowPeak 保持一致
        chrom = row.get('genomic_accession') or row.get('chromosome') or ''
        strand = row.get('strand', '+').strip() or '+'
        locus = row.get('locus_tag', '').strip()
        gene = row.get('symbol', '').strip()
        name = row.get('name', '').strip()
        # 标准名优先使用映射，其次构造前缀
        candidates = [locus, gene, name, f"Kmarxianus_{locus}" if locus else None]
        gene_std = None
        for cand in candidates:
            if cand and cand in gene_mapping:
                gene_std = gene_mapping[cand]
                break
        if gene_std is None:
            gene_std = f"Kmarxianus_{locus}" if locus else (gene or name or '')
        start0 = max(0, start_1 - 1)
        end0 = end_1
        tss = start0 if strand == '+' else max(start0, end0 - 1)
        gene_pos.append({
            'gene_name': gene_std,
            'chrom': chrom,
            'strand': strand,
    ...
    print(f"feature_table基因数: {len(gene_pos)}")
...
def load_gene_mapping():
    """从GFF构建基因映射（兼容 locus_tag / gene / Name / 前缀形式）。"""
    print("加载基因映射信息 (GFF)...")
    mapping = {}
    if not GFF_FILE or not os.path.exists(GFF_FILE):
        print("⚠️ 未提供GFF_FILE，返回空映射")
        return mapping
    ...
    with open(GFF_FILE, 'r') as f:
        for line in f:
            ...
            locus = attrs.get('locus_tag') or attrs.get('ID') or ''
            gene = attrs.get('gene') or ''
            name = attrs.get('Name') or ''
            canonical = f"Kmarxianus_{locus}" if locus else gene or name
            aliases = [locus, gene, name, canonical]
            for alias in aliases:
                if alias:
                    mapping[alias] = canonical
                    if not alias.startswith('Kmarxianus_') and canonical:
                        mapping[f"Kmarxianus_{alias}"] = canonical
    print(f"基因映射数量: {len(mapping)} (GFF)")
    return mapping
...
def create_condition_encoder(expr_gsms, use_fixed_mapping=False, fixed_mapping_info=None):
    ...
    cond_df.columns = [_norm_col(c) for c in cond_df.columns]
    ...
    # 根据配置跳过指定列（同步规范列名）
    skip_columns = [_norm_col(c) for c in ENCODING_CONFIG["skip_columns"]]
    one_hot_cols = [_norm_col(c) for c in ENCODING_CONFIG["one_hot_columns"]]
    min_max_cols = [_norm_col(c) for c in ENCODING_CONFIG["min_max_columns"]]
    ...
    # 确保所有需要的列都存在
    available_one_hot = [col for col in one_hot_cols if col in cond_df_for_encoding.columns]
    available_min_max = [col for col in min_max_cols if col in cond_df_for_encoding.columns]
...
def encode_with_fixed_mapping(df, mapping_info):
    """使用固定映射（encoding_mapping_info.json）对条件数据进行编码。"""
    df = df.copy()
    df.columns = [_norm_col(c) for c in df.columns]

    # 构造规范列名到原始映射列名的映射
    norm_to_orig = {_norm_col(col): col for col in mapping_info.get('column_order', [])}
    feature_map = mapping_info.get('feature_mapping', {})
    total_dim = mapping_info.get('total_features', 0)

    encoded = np.zeros((len(df), total_dim), dtype=np.float32)

    for norm_col, orig_col in norm_to_orig.items():
        if orig_col not in feature_map:
            continue
        info = feature_map[orig_col]
        if norm_col in df.columns:
            col_vals = df[norm_col]
        else:
            # 缺失列直接跳过，保持全零
            continue

        if info.get('type') == 'one_hot':
            mapping = info.get('mapping', {})
            for i, v in enumerate(col_vals.astype(str)):
                idx = mapping.get(str(v))
                if idx is not None:
                    encoded[i, idx] = 1.0
        elif info.get('type') == 'min_max':
            min_v = info.get('min_value', 0.0)
            max_v = info.get('max_value', 1.0)
            feat_idx = info.get('feature_idx')
            if feat_idx is None:
                continue
            # 转数值并处理时间/字符串
            def to_numeric(x):
                ...
                if isinstance(x, str):
                    xs = x.strip()
                    if '时间' in orig_col or 'time' in orig_col.lower():
                        if xs.endswith('min'):
                            try:
                                return float(xs.replace('min', '')) * 60
                            except Exception:
                                return 0.0
                        if xs.endswith('s'):
                            try:
                                return float(xs.replace('s', ''))
                            except Exception:
                                return 0.0
                    # 提取数字
                    import re as _re
                    m = _re.search(r"(-?\d+\.?\d*)", xs)
                    if m:
                        try:
                ...
                try:
                    ...
                    return 0.0
            vals = col_vals.apply(to_numeric).astype(float)
            if max_v == min_v:
                norm_vals = np.zeros_like(vals, dtype=np.float32)
            else:
                norm_vals = ((vals - min_v) / (max_v - min_v)).clip(0, 1).astype(np.float32)
            encoded[:, feat_idx] = norm_vals
        else:
            continue

    print(f"使用固定映射编码完成: {encoded.shape}")
    return encoded
...
def get_summit_positions(matrix_file, summit_file):
    """获取peak的summit位置信息
    优先使用 narrowPeak：chrom, start, end, name, ..., summit_offset(col 9)
    若缺失 summit_offset，则用 (start+end)//2。
    名称优先匹配 name->peak_id，否则按行序对齐。
    """
    ...
    if summit_file and os.path.exists(summit_file):
        ...
        summit_df.columns = [f'c{i}' for i in range(summit_df.shape[1])]
        # 构造 name → (chrom, summit_pos) 映射
        name_to_pos = {}
        for _, row in summit_df.iterrows():
            chrom = str(row['c0'])
            try:
                start = int(row['c1'])
                end = int(row['c2'])
                ...
                continue
            name = str(row['c3']) if 'c3' in row else ''
            try:
                summit_offset = int(row['c9']) if 'c9' in row else None
            except Exception:
                summit_offset = None
            summit_pos = start + summit_offset if summit_offset is not None else (start + end) // 2
            name_to_pos[name] = (chrom, summit_pos)

        summit_chroms, summit_positions = [], []
        for i, peak_id in enumerate(matrix_peak_ids):
            if peak_id in name_to_pos:
                chrom, pos = name_to_pos[peak_id]
            elif i < len(summit_df):
                # 回退按行序对齐
                chrom = str(summit_df.iloc[i]['c0'])
                try:
                    start = int(summit_df.iloc[i]['c1'])
                    end = int(summit_df.iloc[i]['c2'])
                except Exception:
                    start = end = 0
                try:
                    summit_offset = int(summit_df.iloc[i]['c9']) if 'c9' in summit_df else None
                except Exception:
                    summit_offset = None
                pos = start + summit_offset if summit_offset is not None else (start + end) // 2
            else:
                chrom, pos = 'unknown', 0
            summit_chroms.append(chrom)
            summit_positions.append(pos)
...
def main():
    """主函数：构建可训练的numpy文件"""
    log_writer = _setup_logging()
    _console_print("开始构建 Numpy 训练数据")
    _console_print(f"输出目录: {OUTPUT_DIR}")
    _console_print(f"日志文件: {log_file}")
    print("\n" + "="*60)
    ...
    for idx, item in enumerate(MATRIX_SUMMIT_LIST, 1):
        ...
        _console_print(f"[{idx}/{len(MATRIX_SUMMIT_LIST)}] 处理: {sample_name}")
        ...
        try:
            ...
            print(f"\n   ✅ {sample_name} 处理完成")
            _console_print(f"[{idx}/{len(MATRIX_SUMMIT_LIST)}] 完成: {sample_name}")
            
        except Exception as e:
            print(f"\n   ❌ 处理失败: {e}")
            import traceback
            traceback.print_exc()
            _console_print(f"[{idx}/{len(MATRIX_SUMMIT_LIST)}] 失败: {sample_name}（详见日志）")
            continue
    ...
    print(f"📝 日志文件: {log_file}")
    print("="*60 + "\n")
    _console_print("构建流程完成")
    _console_print(f"输出目录: {OUTPUT_DIR}")
    _console_print(f"日志文件: {log_file}")
    
    # 关闭日志文件
    if isinstance(sys.stdout, _LogWriter):
        sys.stdout.close()
    if isinstance(sys.stderr, _LogWriter):
        sys.stderr.close()
