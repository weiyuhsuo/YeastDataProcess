...
GENE_INFO_FILE = r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\Saccharomyces_cerevisiae.gene_info'
...
TSS_BED_FILE = r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\Sc_EPDnew_cleaned.bed'
...
ANNOT_FILE = r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\ncbiRefSeqCurated.txt'

# 4. 表达矩阵文件（EXPR_FILE）- 必需
#    用途：获取所有样本的基因表达值（用于分配到peaks）
#    格式：CSV格式，第一列是基因ID（行索引），后续列是样本（列名以GSM开头）
#    说明：表达值应该是TPM格式，脚本会进行log2(TPM+1)转换
EXPR_FILE = r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\sample\训练_第三批数据_表达矩阵.csv'
...
COND_FILE = r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\sample\2592ver2_训练_第三批数据_样品信息_补默认.xlsx'
...
OUTPUT_BASE_DIR = r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\output'
...
MATRIX_SUMMIT_LIST = [
    {
        "matrix": r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\ATAC1_ver2_matrix.csv',
        "summit": r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\ATAC1_ver2.narrowPeak'
    },
    # 可以添加更多样本，例如：
    # {
    #     "matrix": r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\ATAC2_matrix.csv',
    #     "summit": r'd:\BaiduNetdiskDownload\YeastDataProcess\mac-yeast-data-process\BuildNumpy\data\ATAC2_peaks.narrowPeak'
    # },
]
...
class Logger:
    ...
    def write(self, message):
        ...
        try:
            ...
            # 如果终端无法编码某些字符，尝试替换或跳过
            try:
                # 尝试用 errors='replace' 替换无法编码的字符
...
def create_condition_encoder(expr_gsms):
    """创建条件编码器（基于筛选后的数据）"""
    print("创建条件编码器...")
...
def encode_conditions(df, preprocessor):
    """使用编码器编码条件数据"""
    print("编码条件数据...")
...
def assign_expression_to_peaks_weighted(gene_pos_df, expr_data, chroms, summit_positions, sigma=500):
    ...
    print(f"总基因数: {len(expr_data.index)}, 有效基因数: {len(valid_genes)}")
    ...
    print(f"✅ 能分配表达值的基因数: {matched_gene_count} (来自{len(valid_genes)}个有效表达基因)")
    print(f"✅ 分配统计: 单peak基因 {single_peak_assignments} 个, 多peak基因 {multi_peak_assignments} 个")
    print(f"✅ 总gene-peak连接数: {total_assignments}")
    print(f"✅ 平均每个基因对应: {total_assignments/matched_gene_count:.2f} 个peaks")
...
def main():
    ...
    all_expr_gsms = [x for x in expr_data_head.columns[1:] if x.startswith('GSM')]
    print(f"   ✅ 表达数据GSM数量: {len(all_expr_gsms)}")
    
    # 若条件文件缺失，走精简路径：仅计算并输出mapping统计
    if not os.path.exists(COND_FILE):
        print(f"\n⚠️ 未找到条件文件: {COND_FILE}")
        print("→ 进入精简流程：仅生成映射统计\n")
    ...
    # 3. 创建条件编码器（基于筛选后的数据）
    print("\n▶ 步骤 3/8: 创建条件编码器...")
    preprocessor, expected_cond_dim = create_condition_encoder(expr_gsms)
    ...
    cond_encoded = encode_conditions(cond_df, preprocessor)
