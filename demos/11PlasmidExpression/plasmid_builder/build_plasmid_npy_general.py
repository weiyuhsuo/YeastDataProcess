...
class PlasmidConfig:
    """质粒配置类"""
    def __init__(self, name, fimo_file, expression_file, sample_info_file, 
    ...
    def get_output_filename(self):
        """生成输出文件名"""
        base_name = f"{self.name.lower()}_plasmid_expression"
        if self.promoter_name:
            base_name += f"_{self.promoter_name}"
        base_name += f"_copy{self.copy_number}"
        return base_name
...
def load_motif_scores_from_fimo(fimo_file):
    """从fimo结果文件加载motif得分"""
    print(f"从fimo结果文件加载motif得分: {fimo_file}")
    
    if not os.path.exists(fimo_file):
        print(f"错误: fimo结果文件不存在: {fimo_file}")
        return None
    
    # 读取fimo结果，明确指定分隔符
    fimo_df = pd.read_csv(fimo_file, sep='\t', comment='#')
    print(f"加载fimo结果: {len(fimo_df)} 行")
...
def build_motif_matrix(motif_ids, motif_scores, normalize_func, copy_number):
    """构建motif矩阵（283维）- 使用fimo扫描结果，乘以拷贝数模拟拷贝数影响"""
    print(f"构建motif矩阵...")
    print(f"注意：motif得分将乘以{copy_number}来模拟{copy_number}拷贝数的影响")
    ...
    for i, motif_id in enumerate(motif_ids):
        if motif_id in motif_scores:
            # 使用真实的motif得分，乘以拷贝数模拟拷贝数影响
            score = motif_scores[motif_id]
            score_copied = score * copy_number
            normalized_score = normalize_func(score_copied)
            motif_matrix[0, 0, i] = normalized_score
            print(f"  {motif_id}: 原始得分 {score:.4f} -> {copy_number}x {score_copied:.4f} -> 归一化 {normalized_score:.4f}")
        else:
    ...
    print(f"注意：motif得分已乘以{copy_number}来模拟{copy_number}拷贝数的影响")
    return motif_matrix
...
def build_expression_vector(expression_df, target_gene):
    """构建表达值向量（1维，目标基因保持原始值，不除以拷贝数）"""
    print(f"构建表达值向量，目标基因: {target_gene}")
    ...
    for col in expression_columns:
        try:
            value = float(expression_df[col].iloc[0])  # 第一行是目标基因的表达值
    ...
    # 重塑为三维矩阵
    num_samples = len(expression_values)
    ...
    return expression_values, num_samples

def encode_conditions_with_exported_encoder(df, feature_list):
    """使用导出的编码器编码条件数据"""
...
def load_sample_info(sample_info_file):
    ...
    if not os.path.exists(sample_info_file):
        print(f"错误: 样本信息文件不存在: {sample_info_file}")
        return None
...
def build_final_matrix(motif_matrix, accessibility, conditions, expression, num_samples):
    """构建最终的矩阵"""
    print("构建最终矩阵...")
    ...
    # accessibility: (1, 1, 1)  
    # conditions: (num_samples, 1, 60)
    # expression: (num_samples, 1, 1)
    
    # 首先将motif和accessibility复制到num_samples个样本
    motif_matrix_expanded = np.tile(motif_matrix, (num_samples, 1, 1))  # (num_samples, 1, 283)
    accessibility_expanded = np.tile(accessibility, (num_samples, 1, 1))  # (num_samples, 1, 1)
    ...
    final_matrix = np.concatenate([
        motif_matrix_expanded,  # (num_samples, 1, 283)
        accessibility_expanded, # (num_samples, 1, 1)
        conditions,             # (num_samples, 1, 60)
        expression             # (num_samples, 1, 1)
    ], axis=2)  # 在第三个维度上拼接
...
def build_plasmid_npy(config):
    """构建质粒npy文件的主函数"""
    print(f"开始构建{config.name}质粒表达npy文件...")
    print(f"配置信息:")
    print(f"  Fimo文件: {config.fimo_file}")
    print(f"  表达数据: {config.expression_file}")
    print(f"  样本信息: {config.sample_info_file}")
    print(f"  目标基因: {config.target_gene}")
    print(f"  拷贝数: {config.copy_number}")
    ...
    # 2. 加载数据
    expression_df = load_expression_data(config.expression_file)
    if expression_df is None:
        return False
    
    sample_df = load_sample_info(config.sample_info_file)
    ...
    motif_scores = load_motif_scores_from_fimo(config.fimo_file)
    motif_matrix = build_motif_matrix(motif_ids, motif_scores, normalize_with_global_range, config.copy_number)
    accessibility = build_accessibility_vector()
    expression, num_samples = build_expression_vector(expression_df, config.target_gene)
    
    # 使用导出的编码信息进行特征编码
    conditions = encode_conditions_with_exported_encoder(sample_df, feature_list)
    # 重塑为三维矩阵
    ...
    # 5. 构建最终矩阵
    final_matrix = build_final_matrix(motif_matrix, accessibility, conditions, expression, num_samples)
    
    # 6. 重塑为(num_samples, 1, 345)的格式
    final_npy = final_matrix.reshape(num_samples, 1, 345)
    print(f"最终npy形状: {final_npy.shape}")
    
    # 7. 保存npy文件
    output_filename = config.get_output_filename()
    output_file = f"data/{config.name}/{output_filename}.npy"
    ...
    # 8. 保存详细信息
    info_file = f"data/{config.name}/{output_filename}_info.txt"
    with open(info_file, 'w') as f:
        f.write(f"{config.name}质粒表达npy文件信息\n")
        f.write("=" * 50 + "\n")
        ...
        f.write(f"- 三维矩阵结构: ({num_samples}, 1, 345)\n")
        f.write("- 使用训练数据的编码器确保格式一致\n")
        f.write(f"- Motif得分乘以{config.copy_number}模拟{config.copy_number}拷贝数的影响\n")
        f.write("- 表达值不除以拷贝数，保持原始水平\n")
        f.write(f"- 使用{config.name}的fimo扫描结果\n")
        f.write(f"- 基因: {config.target_gene}\n")
        if config.promoter_name:
            f.write(f"- 启动子: {config.promoter_name}\n")
