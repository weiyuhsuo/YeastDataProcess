...
def create_condition_encoder_from_exported_info():
    """使用导出的编码信息创建条件编码器"""
    print("使用导出的编码信息创建条件编码器...")
    ...
    # 分离数值和分类特征
    numeric_features = feature_list[feature_list['feature_type'] == 'numerical']
    categorical_features = feature_list[feature_list['feature_type'] == 'categorical']
    
    print(f"数值特征数量: {len(numeric_features)}")
    print(f"分类特征数量: {len(categorical_features)}")
    
    # 创建编码器
    numeric_cols = numeric_features['feature_name'].tolist()
    categorical_cols = categorical_features['original_column'].unique().tolist()
    ...
    print(f"编码器创建完成，预期特征数量: {len(feature_list)}")
    return preprocessor, len(feature_list)

def load_motif_scores_from_fimo():
    """从fimo结果文件加载motif得分"""
    print("从fimo结果文件加载motif得分...")
...
def build_motif_matrix(motif_ids, motif_scores, normalize_func):
    """构建motif矩阵（283维）- 使用真实的fimo扫描结果，乘以100模拟100拷贝数"""
    print("构建motif矩阵...")
    print("注意：motif得分将乘以100来模拟100拷贝数的影响")
    ...
    for i, motif_id in enumerate(motif_ids):
        if motif_id in motif_scores:
            # 使用真实的motif得分，乘以100模拟100拷贝数
            score = motif_scores[motif_id]
            score_100x = score * 100.0  # 乘以100模拟100拷贝数
            normalized_score = normalize_func(score_100x)
            motif_matrix[0, 0, i] = normalized_score
            print(f"  {motif_id}: 原始得分 {score:.4f} -> 100x {score_100x:.4f} -> 归一化 {normalized_score:.4f}")
        else:
    ...
    print(f"注意：motif得分已乘以100来模拟100拷贝数的影响")
    return motif_matrix
...
def build_expression_vector(expression_df):
    """构建表达值向量（1维，rip1/YEL024W保持原始值，不除以100）"""
    print("构建表达值向量...")
    
    # 跳过第一列gene，使用实际的GSM样本表达值列
    expression_columns = [col for col in expression_df.columns if col != 'gene']
    print(f"找到 {len(expression_columns)} 个GSM样本列: {expression_columns}")
    
    # 提取表达值，保持原始值，然后进行log1p转换
    expression_values = []
    for col in expression_columns:
        try:
            value = float(expression_df[col].iloc[0])  # 第一行是YEL024W的表达值
            # 保持原始值，不除以100，然后进行log1p转换
...
def encode_conditions_with_exported_encoder(df, preprocessor, feature_list):
    ...
    for feature_idx, (_, feature_info) in enumerate(feature_list.iterrows()):
        ...
        transformer = feature_info['transformer']
        ...
        encoded_pos = feature_info['encoded_position']
...
def build_final_matrix(motif_matrix, accessibility, conditions, expression):
    ...
    # 首先将motif和accessibility复制到12个样本
    motif_matrix_expanded = np.tile(motif_matrix, (12, 1, 1))  # (12, 1, 283)
    accessibility_expanded = np.tile(accessibility, (12, 1, 1))  # (12, 1, 1)
    ...
    final_matrix = np.concatenate([
        motif_matrix_expanded,  # (12, 1, 283)
        accessibility_expanded, # (12, 1, 1)
        conditions,             # (12, 1, 60)
        expression             # (12, 1, 1)
    ], axis=2)  # 在第三个维度上拼接
...
def build_npy_file():
    ...
    motif_scores = load_motif_scores_from_fimo()
    ...
    # 7. 保存npy文件
    output_file = "data/plasmid_expression_correct.npy"
    np.save(output_file, final_npy)
    print(f"npy文件已保存: {output_file}")
    
    # 8. 保存详细信息
    info_file = "data/plasmid_expression_correct_info.txt"
    with open(info_file, 'w') as f:
        f.write("正确的质粒表达npy文件信息\n")
        f.write("=" * 50 + "\n")
        ...
        f.write("- 表达值使用真实GSM样本数据，保持原始值，进行log1p转换\n")
        f.write("- 使用60维实验条件编码（7个数值+53个分类）\n")
        f.write("- 三维矩阵结构: (12, 1, 345)\n")
        f.write("- 使用训练数据的编码器确保格式一致\n")
        f.write("- Motif得分乘以100模拟100拷贝数的影响\n")
        f.write("- 表达值不除以100，保持原始水平\n")
