"""
BuildNumpyinorder.py - 构建训练用的 NPZ（workflow260508/4matrix3D）

与历史 BuildNumpy 的区别：
  - 表达矩阵、TSS BED、refFlat 注释中的基因名已统一为标准 ID，不再读取 gene_info 做别名映射。
  - 路径默认相对本脚本目录；按需修改「文件路径配置」块即可。

功能说明：
  1. 读取 peak 矩阵（motif + accessibility）
  2. 读取基因表达与实验条件
  3. 将表达分配到 peaks（TSS 距离加权）
  4. 条件特征：独热 + Min-Max
  5. 输出 .npz 等到带时间戳的输出目录
"""
...
# 1. TSS BED（基因名已与表达矩阵一致）
TSS_BED_FILE = os.path.join(_DATA_DIR, 'Sc_EPDnew_cleaned_standardized.bed')

# 2. 备用 refFlat 注释（列与 ncbiRefSeqCurated 相同；name2 为标准基因名）
ANNOT_FILE = os.path.join(_DATA_DIR, 'ncbiRefSeqCurated_standardized.txt')

# 3. 表达矩阵（首列为 GeneID，列名为 GSM… 等样本 ID）
EXPR_FILE = os.path.join(_DATA_DIR, 'SampleData', 'expression_matrix_standardized.csv')

# 4. 样本条件表（.xlsx，含 GSM 列）。若暂无文件，设为 None：脚本会用零向量占位条件维。
#    请把对应 xlsx 放到本目录或下面路径指向你的文件。
_COND_CANDIDATES = [
    os.path.join(_DATA_DIR, 'SampleData', 'sample_conditions.xlsx'),
    '/home/rhys/YeastDataProcess/4BuildNumpy/135sample/135_sample_260121.xlsx',
]
COND_FILE = next((p for p in _COND_CANDIDATES if os.path.isfile(p)), None)

# 5. 外部 peak–gene 标签 CSV（可选）
LABELS_FILE = None

# 6. 固定条件编码 JSON（推理对齐用）。新训练建议 None，由当前条件表生成 encoding_mapping_info.json。
FIXED_MAPPING_FILE = None

# --- 输出目录 ---
OUTPUT_BASE_DIR = os.path.join(_SCRIPT_DIR, 'output')
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
OUTPUT_DIR = os.path.join(OUTPUT_BASE_DIR, f"run_{timestamp}")
os.makedirs(OUTPUT_DIR, exist_ok=True)
log_file = os.path.join(OUTPUT_DIR, "build_numpy.log")

# --- Peak 矩阵 + narrowPeak（matrix 由上游 3matrix 等步骤生成）---
_MATRIX_DEFAULT = os.path.join(
    _SCRIPT_DIR, '..', '3matrix', 'output', '260115', '20260115_193716', 'ATAC1_matrix.csv'
)
MATRIX_SUMMIT_LIST = [
    {
        "matrix": os.path.normpath(_MATRIX_DEFAULT),
        "summit": os.path.join(_DATA_DIR, 'fine_s90_e100_peaks.narrowPeak'),
    },
]
...
def load_gene_positions():
    """从 BED 加载 TSS；不在 BED 中的基因从 refFlat 用 CDS±SHIFT_BP 补全（基因名已统一，不做别名映射）。"""
    print(f"从 BED 加载 TSS: {TSS_BED_FILE}")
    gene_pos = []
    tss_count = 0
    ...
    print(f"BED 行数（含重复转录本则多行）: {len(gene_pos)}；TSS 规则行: {tss_count}")

    print(f"从备用注释补全不在 BED 集合中的基因: {ANNOT_FILE}")
    annot_count = 0
    mapped_standard_genes = {item['gene_name'] for item in gene_pos}
    ...
    with open(ANNOT_FILE, 'r') as f:
        for line in f:
            ...
            if gene_name not in mapped_standard_genes:
                if strand == '+':
                ...
                mapped_standard_genes.add(gene_name)
                annot_count += 1
                cds_fallback_count += 1

    print(f"从注释补充: {annot_count}；其中 CDS±{SHIFT_BP} 近似 TSS: {cds_fallback_count}")
    print(f"最终 gene_pos 行数: {len(gene_pos)}")
...
def compute_and_save_mapping_stats(expr_file, gene_pos_df, out_dir):
    """统计表达矩阵基因在 gene_pos 中的来源（TSS BED / CDS 回推 / 无位置）。"""
    print("\n" + "="*60)
    print("   基因位置来源统计（TSS / CDS±93 / 未映射）")
    print("="*60)
    try:
    ...
    unique_genes = sorted({str(g).strip() for g in expr_df.index})
...
def main():
    ...
    # index_col=0 时基因列为索引，剩余列均为样本；不再假定需跳过「第 0 列」
    all_expr_gsms = [str(x) for x in expr_data_head.columns if str(x).startswith('GSM')]
    if len(all_expr_gsms) == 0:
        all_expr_gsms = [str(x) for x in expr_data_head.columns]
        print(f"   ⚠️ 警告: 未找到以 'GSM' 开头的列，使用表达矩阵全部数值列")
    print(f"   ✅ 表达数据 GSM（或全部样本列）数量: {len(all_expr_gsms)}")

    cond_available = bool(COND_FILE) and os.path.isfile(COND_FILE)
    if not cond_available:
        print(f"\n⚠️ 未找到条件文件（COND_FILE={COND_FILE!r}）")
        print("→ 将跳过条件特征，使用零向量占位，保持 GSM 顺序与表达矩阵一致\n")
    ...
    # 5. 加载基因位置（名称已统一，无 gene_info 映射）
    print("\n▶ 步骤 5/8: 加载基因位置...")
    gene_pos_df = load_gene_positions()
    print(f"   ✅ 基因位置表行数: {gene_pos_df.shape[0]}")

    # 6. 表达基因 × 位置来源统计
    compute_and_save_mapping_stats(EXPR_FILE, gene_pos_df, OUTPUT_DIR)
